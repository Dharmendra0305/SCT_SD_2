# AI-Inspired Number Guessing Game 🤖

![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=java&logoColor=white)
![Status](https://img.shields.io/badge/Status-Completed-success?style=for-the-badge)
![Internship](https://img.shields.io/badge/Internship-SkillCraft_Technology-blue?style=for-the-badge)

## 📌 Overview
This project is an AI-inspired "Guess the Number" graphical game developed as Task 02 during my software development internship at **SkillCraft Technology**. 

Instead of basic "high/low" responses, the application uses **heuristic feedback logic** to analyze the user's proximity to the target number, providing adaptive hints (e.g., "You're very close!"). It demonstrates proficiency in Object-Oriented Programming (OOP), event-driven architecture, and UI/UX design using Java Swing.

## ✨ Features
* **Adaptive AI Hints:** Heuristic logic provides context-aware feedback based on the absolute difference between the guess and the target.
* **Graphical User Interface (GUI):** Clean, modern interface built entirely with Java Swing.
* **Dynamic Difficulty Levels:** Choose between Easy (1-10), Medium (1-50), and Hard (1-100).
* **Robust Error Handling:** Input validation prevents crashes from non-numeric entries.
* **Session Tracking:** Real-time attempt counters and game-over state management.

## 🛠️ Tech Stack
* **Language:** Java (JDK 8 or higher)
* **Framework:** Java Swing (GUI components)
* **Libraries:** `java.awt` (Event handling and layout), `java.util.Random` (Number generation)

## 🚀 Installation & Setup

1. **Clone the repository:**
   ```bash
   git clone [https://github.com/Dharmendra0305/SCT_SD_2.git](https://github.com/Dharmendra0305/SCT_SD_2.git)

2. **Navigate to the directory:**
   ```bash
   cd SCT_SD_2\src

3. **Compile the Java file:**
   ```bash
   javac GuessingGame.java

4. **Run the application:**
   ```bash
   java GuessingGame

## 🎮 Usage Example
1. Launch the application.
2. Select your desired difficulty from the dropdown menu.
3. Enter your guess in the text field and click "Guess".
4. Read the adaptive feedback and adjust your next guess accordingly!

🤝 Contribution Guidelines
- Contributions, issues, and feature requests are welcome!
  1. Fork the Project
  2. Create your Feature Branch (git checkout -b feature/AmazingFeature)
  3. Commit your Changes (git commit -m 'Add some AmazingFeature')
  4. Push to the Branch (git push origin feature/AmazingFeature)
  5. Open a Pull Request

## 📄 License
Distributed under the MIT License. See [LICENSE](LICENSE) for more information.
